...
BEGIN
    action_type_id := (SELECT "id" FROM "log_action_types" WHERE name = 'account');
    ...
    CASE TG_TABLE_NAME
        ...
        WHEN 'doctors' THEN
            user_id := COALESCE(NEW.user_id, OLD.user_id);
            qualification_name := (SELECT "name" FROM "doctor_qualifications" WHERE "id" = COALESCE(NEW.qualification_id, OLD.NEW.qualification_id));
            if qualification_name is NULL THEN
                RAISE EXCEPTION 'Qualification not found';
            END IF; 
            specialization_name := (SELECT "name" FROM "doctor_specializations" WHERE "id" = COALESCE(NEW.specialization_id, OLD.specialization_id));
            if specialization_name is NULL THEN
                RAISE EXCEPTION 'Specialization not found';
            END IF; 
            action_description := CASE TG_OP
                WHEN 'INSERT' THEN 'New doctor created: (' || qualification_name || ') (' || specialization_name || ')'
                WHEN 'UPDATE' THEN 'Doctor info updated: (' || qualification_name || ') (' || specialization_name || ')'
                WHEN 'DELETE' THEN 'Doctor deleted: (' || qualification_name || ') (' || specialization_name || ')'
                ELSE NULL
            END;
        WHEN 'patients' THEN
            user_id := COALESCE(NEW.user_id, OLD.user_id);
        ...
        WHEN 'patients_anthropometry' THEN
            patient_id := (SELECT "patient_id" FROM "patients_anthropometry" WHERE "id" = COALESCE(NEW.id, OLD.id));
            if patient_id IS NULL THEN
                RAISE EXCEPTION 'Patient not found'; 
            END IF;
            user_id := (SELECT "user_id" FROM "patients" WHERE "id" = patient_id);
            action_description := CASE TG_OP
    ...
    IF action_description IS NOT NULL THEN
        ...
        VALUES (action_type_id, action_description, user_id);
    END IF;
...
BEGIN
    ...
    log_user_id := COALESCE(
        (SELECT user_id FROM "patinets" WHERE id = NEW.patient_id),
        (SELECT user_id FROM "patinets" WHERE id = OLD.patient_id)
    );
...
BEGIN
    ...
    END IF;
    status_name := (SELECT "name" FROM "payment_statuses" WHERE "id");
    if status_name is NULL THEN
        RAISE EXCEPTION 'Status not found';
    END IF;
	IF (TG_OP = 'INSERT') THEN
...
BEGIN
    SELECT "id" INTO v_not_paid_status_id
    ...
    SELECT dv.service_fee, dv.q_mult
    INTO v_doctor_specialization_fee, v_doctor_qualification_multiplier
    FROM DoctorsView dv
    WHERE dv.doctor_id = NEW.doctor_id;
    ...
    SELECT COALESCE(SUM(m.price * p.dosage / 100), 0)
    INTO v_medications_cost
    FROM prescriptions p
    JOIN medications m ON p.medication_id = m.id
    WHERE p.medical_record_id = NEW.id;
    ...
    VALUES (v_total_bill, CURRENT_DATE, v_not_paid_status_id, NEW.patient_id);
    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in create_bill_after_medical_record: %', SQLERRM;
        RETURN NULL;
END;
$$ LANGUAGE plpgsql;
